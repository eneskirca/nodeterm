 ...
 * check. Reuses `createdAgentHarnessId` (the same derivation the canvas's `linkEndpointOf` and
...
export function linkKindEndpointOf(node: CanvasNodeState): LinkKindEndpoint {
  ...
  return {
    kind: node.kind,
    contextCapable: !sticky && !!createdAgentHarnessId(node) && canContextLink(createdAgentHarnessId(node)!)
  }
}
