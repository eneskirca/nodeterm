...
export function Canvas() {
  ...
  // Latest `travelToProject` for the grouped-recovery batch — the SAME once-mounted-closure reason
  // as `closeStoredNodesRef`/`worktreeControlRef` below: `considerModelAvailability` is defined far
  // above `travelToProject`, so the batch cannot close over the callback directly. This travel is
  // the USER'S OWN click (Retry / "Retry failed" / return-home after a multi-project batch), not an
  // agent verb: the dispatch-level ban on travelling (`closeStoredNodesRef`'s comment) is about
  // agent control taking a screen the human did not offer, and never covered a flow the human
  // clicked. Keep this ref narrow — do not hoist it into the agent-control handler.
  const modelSwitchTravelRef = useRef<(projectId: string) => void>(() => {})
  ...
  const considerModelAvailability = useCallback((): void => {
    ...
    const runRestart = async (target: string): Promise<void> => {
      ...
      const repairOne = async (
        ...
        try {
          ...
          if (
            ...
            modelSwitchTravelRef.current(item.ownerProjectId)
      ...
      if (traveled && homeProjectId && useProjects.getState().activeProjectId !== homeProjectId) {
        ...
        modelSwitchTravelRef.current(homeProjectId)
      }
...
// Delete a session from the board — same confirm + teardown as the canvas Delete key.
  ...
  // Same publish for the grouped-recovery batch's travel (declared with the other model-switch
  // refs near the top of the component).
  useEffect(() => {
    modelSwitchTravelRef.current = travelToProject
  })
