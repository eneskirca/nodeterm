...
import { assembleResumeCommand, type ResumeInputs } from '@shared/agents/launch'
...
/** Wake in the existing shell, retaining the node's model selection. Requiring the model field
 * makes callers explicitly carry it even when the node uses the agent's default (undefined). */
export function assembleWakeResumeCommand(
  inputs: Omit<ResumeInputs, 'sharedIdentity'> & { model: string | undefined },
  env: Record<string, string | undefined>
) {
  return assembleResumeCommand({ ...inputs, sharedIdentity: false }, env)
}
...
export async function performResumePhase(d: {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    try {
      ...
      // settle.
      cancelDelivery = deliverCommand(
        d.io,
        cmd,
        (outcome) => settle(outcome === 'submitted'),
