...
/** The project a chip's node belongs to, when that is not necessarily the active one — the Omni
 *  board renders every open project's lane. Absent = the active project (canvas, per-project board). */
export const GitHubLinkProject = createContext<string | undefined>(undefined)
...
export function GitHubLinkChip({ nodeId, links, variant }: Props) {
  const scoped = useContext(GitHubLinkProject)
  const repository = useProjects((s) =>
    linkRepository(s.projects.find((p) => p.id === (scoped ?? s.activeProjectId))?.kanban))
  ...
  return (
    <ChipBody nodeId={nodeId} links={links} variant={variant} repository={repository} scoped={scoped} />
  )
}

function ChipBody({
  nodeId, links, variant, repository, scoped
}: Props & { repository: string; scoped: string | undefined }) {
  const { api } = useSession()
  const active = useProjects((s) => s.activeProjectId)
  const projectId = scoped ?? active
  ...
  const linkRows = (link: GitHubLink): MenuItem[] => [
    { label: `Open #${link.number} details`, onClick: () => openGitHubLinkDetails(link, scoped) },
    {
    ...
    },
    {
      ...
      danger: true,
      onClick: () => detachGitHubLink(nodeId, link, scoped)
    }
  ]
  ...
  const items: MenuItem[] = [
    ...
    { type: 'separator' },
    {
      ...
      onClick: () => menu && openGitHubLinkPicker(nodeId, menu, scoped)
    }
  ]
